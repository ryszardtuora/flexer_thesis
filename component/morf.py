...
FREQ_FILE = "freq-nkjp1m.txt"
DIMINUTIVES_FILE = "combined.tab"

Token.set_extension("properness", default=[], force=True)
Token.set_extension("style", default=[], force=True)
Token.set_extension("disambiguator", default="", force=True)
Token.set_extension("is_ign", default=True, force=True)
Token.set_extension("freq", default=0, force=True)
Token.set_extension("is_diminutive", default=False, force=True)
Token.set_extension("diminutive_chain", default=[], force=True)
...
class Morfeusz(Pipe):
    ...
    def get_best_analysis(self, tok):
        ...
        ranking = sorted(ranking, key=lambda seg: self.tag_distance(tag.split(":"), seg["tag"].split(":")))
        best = ranking[0]
        return best
